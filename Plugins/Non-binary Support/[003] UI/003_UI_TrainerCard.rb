...
class PokemonTrainerCard_Scene
  ...
  def pbStartScene
    ...
    @viewport.z = 99999
    @sprites = {}
    background_n = pbResolveBitmap(sprintf("Graphics/Pictures/Trainer Card/bg_nonbinary"))
    background_f = pbResolveBitmap(sprintf("Graphics/Pictures/Trainer Card/bg_f"))
    if $Trainer.female? #&& background_f
      addBackgroundPlane(@sprites,"bg","Trainer Card/bg_f",@viewport)
    elsif $Trainer.nonbinary? #&& background_n
      addBackgroundPlane(@sprites,"bg","Trainer Card/bg_nonbinary",@viewport)
    else
      addBackgroundPlane(@sprites,"bg","Trainer Card/bg",@viewport)
    end
    cardexists_n = pbResolveBitmap(sprintf("Graphics/Pictures/Trainer Card/card_nonbinary"))
    cardexists_f = pbResolveBitmap(sprintf("Graphics/Pictures/Trainer Card/card_f"))
    @sprites["card"] = IconSprite.new(0,0,@viewport)
    if $Trainer.female? #&& cardexists_f
      @sprites["card"].setBitmap("Graphics/Pictures/Trainer Card/card_f")
    elsif $Trainer.nonbinary? #&& cardexists_n
